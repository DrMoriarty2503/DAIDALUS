...
def generate_conflict_scenario(
    ...
    alt_c = random.uniform(*ownship_config['altitude_range'])  # Генерация случайной высоты в футах
    ...
    # 3. Движение
    own_heading = random.uniform(*ownship_config['heading_range'])
    ...
    own_vz = random.uniform(*ownship_config['vz_range'])  # Вертикальная скорость в футах

    ac_speed = random.uniform(*ac1_config['speed_knot']) if isinstance(ac1_config['speed_knot'], tuple) else ac1_config[
        'speed_knot']
    ac_heading = random.uniform(*ac1_config['heading_range'])
    ac_vx, ac_vy = heading_to_vx_vy(ac_heading, ac_speed)
    ac_vz = random.uniform(*ac1_config['vz_range'])
